...
def GImapHasExtensions(imapconn):
  ...
  return bool(d[0].decode().count('X-GM-EXT-1'))
  return True
...
def GImapSendID(imapconn, name, version, vendor, contact):
  ...
  return shlex.split(d[0][1:-1].decode())
...
def GImapGetMessageLabels(imapconn, uid):
  ...
  if d[0] != None:
    labels = re.search('^[0-9]* \(X-GM-LABELS \((.*?)\) UID %s\)' % uid, d[0].decode()).group(1)
...
def GImapGetFolders(imapconn):
  ...
  label_mappings = {}
  for line in d:
    flags, delimiter, label_local_name = list_response_pattern.match(line.decode()).groups()
    flags_list = flags.split(' ')
    for flag in flags_list:
      if flag not in ['\\HasNoChildren', '\\HasChildren', '\\Noinferiors']:
